    ...
    <admin-template :current_user="current_user">
        ...
        <div class="bg-white rounded-lg min-h-full py-10 px-10">
            ...
            <div id="header" class="w-full border-b flex items-center gap-4 py-4 border-collapse table-auto w-full text-sm">
                <div class="basis-2/12 font-medium text-slate-500 text-left">Username</div>
                <div class="basis-1/12 font-medium text-slate-500 text-center">Installed</div>
                <div class="basis-3/12 font-medium text-slate-500 text-left">Installation Directory</div>
                <div class="basis-1/12 font-medium text-slate-500 text-left">Role / Type</div>
                <div class="basis-1/12 font-medium text-slate-500 text-right"></div>
            </div>

            <Link class="flex items-center gap-4 block h-10 border-b border-slate-100 text-slate-500 hover:bg-slate-100 transition-colors duration-100 cursor-pointer" v-for="user in all_filtered_users"  :href="$route('admin.usermanagement.user', { user: user.item.username })">
                ...
                <div class="basis-3/12 truncate">
                    {{ user.item.installDir }}
                </div>
                <div class="basis-1/12 truncate">
                    <MemberTypeBadge :type="MemberType[user.item.type]"></MemberTypeBadge>
                </div>
                <div class="basis-3/12 text-slate-600 text-right">
                    <div class="flex">
                        <div class="grow"></div>
                        <button
                            v-if="user.item.type.toUpperCase() === MemberType.applicant.toUpperCase()"
                            class="bg-slate-300 p-1 -my-1 ml-2 rounded px-2 uppercase font-semibold text-xs hover:text-white hover:bg-slate-600 transition-colors">
                            <ChevronUpIcon class="inline-block h-4 w-4 mr-1 -ml-1 text-slate-500"></ChevronUpIcon>
                            Promote to Member
                        </button>
                    <button
                        class="bg-slate-100 p-1 -my-1 ml-2 rounded px-2 uppercase font-semibold text-xs hover:text-white hover:bg-slate-600 transition-colors">
                        Edit
                    </button>
                    </div>
                </div>
...
let fuse = new Fuse(props.all_users, {
    threshold: .4,
    includeScore: true,
    keys: ['username']
})
...
const all_filtered_users = computed(() => {
    return search_term.value !== '' ? fuse.search(search_term.value) : props.all_users.map((user) => ({'item': user}))
})
